package com.app.shopzz.socialmediaintegration;
 ...
 *
 *  TODO : Before implementation, please check for latest dependency or new changes given by facebook team.
 ...
 *
 *          till 26-6-2017 latest version is 4.24.0.
 *
 *         GraphRequest API version 2.9 available until At least July 2019.
...
public class FacebookIntegration {
     ...
     * <p>
     * Go to facebook developer console and login with credentials.
     * <ul>Go to <a href="https://developers.facebook.com">Facebook developer console</a>
     * <li> Go to myApps and select your project</li>
     * <li> Go to "Roles" from your left panel</li>
     * <li> select "Testers" and add testers</li>
     * <li> add facebook id and submit it</li>
     * </ul>
     * <p>
    ...
    public void initFacebookSdk() {
        AppEventsLogger.activateApp(BaseApplication.getInstance());
...
/*    public void initFacebookShareDialog() {
        ...
        fbShareDialog.registerCallback(callbackManager, new FacebookCallback<Sharer.Result>() {
            ...
            public void onSuccess(Sharer.Result result) {
                Debug.trace(BaseApplication.getInstance().getString(R.string.strShareSuccess));
            }

            @Override
            public void onCancel() {
                Debug.trace(BaseApplication.getInstance().getString(R.string.strShareFail));
            }
     ...
     * @see #getFBUserDetails(IFacebookResultInfo)
     */
    public void loginToFaceBook(final IFacebookResultInfo facebookResultInfo) {
        ...
        CustomDialog.getInstance().show(activityContext);
        LoginManager.getInstance().registerCallback(callbackManager, new FacebookCallback<LoginResult>() {
            @Override
            public void onSuccess(LoginResult loginResult) {
                getFBUserDetails(facebookResultInfo);
            ...
            public void onError(FacebookException error) {
                ...
                ToastHelper.displayCustomToast(activityContext, activityContext.getString(R.string.errorMsgLoginFailed));
                CustomDialog.getInstance().hide();
     ...
     */
    private void getFBUserDetails(final IFacebookResultInfo facebookResultInfo) {
        GraphRequest request = GraphRequest.newMeRequest(
                ...
                new GraphRequest.GraphJSONObjectCallback() {
                    ...
                    public void onCompleted(JSONObject object, GraphResponse response) {
                        ...
                        String firstName = object.optString(ApiList.KEY_FIRST_NAME);
                        String lastName = object.optString(ApiList.KEY_LAST_NAME);
                        String email = object.optString(ApiList.KEY_EMAIL);
                        //int registerBy = RegisterBy.FACEBOOK.getType();
                        String socialMediaUserId = object.optString(ApiList.KEY_FACEBOOK_ID);

                        //String fbProfileImageUrl = String.format(activityContext.getString(R.string.strFbProfileImageUrl), socialMediaUserId);

//                        Debug.trace("FirstName:" + firstName);
//                        Debug.trace("LastName:" + lastName);
//                        Debug.trace("Email:" + email);

                        facebookResultInfo.getFacebookResult(object);
                    }
                }
        );
     ...
     * <br>To share link content with facebook. Using shareDialog we can share with facebook no matter
     * that user have installed app in their device or not. It will open web view automatically
     * and after logged in, user can share content with facebook.</br>
     * <pre>{@code
     * public void onClickEvent(View view) {
     * switch (view.getId()) {
     * case R.id.xxx:
     * if (Util.checkInternetConnection()) {
     * FacebookIntegration.shareContentWithDialog(title,contentDescription,imageUrl,contentUrl);
     * } else {
     ...
     * @deprecated since sdk version 4.22.0 onwards
     */
        ...
        if (!TextUtils.isEmpty(imageUrl) && !TextUtils.isEmpty(contentUrl) && URLUtil.isNetworkUrl(imageUrl) && URLUtil.isNetworkUrl(contentUrl)) {
            ...
            if (ShareDialog.canShow(ShareLinkContent.class)) {
                ShareLinkContent shareLinkContent = new ShareLinkContent.Builder()
                        .setContentTitle((!TextUtils.isEmpty(contentDescription)) ? contentTitle : "")
                        .setContentDescription((!TextUtils.isEmpty(contentDescription)) ? contentDescription : "")
     ...
     * case R.id.xxx:
     * if (Util.checkInternetConnection()) {
     * facebookIntegration.shareLinkContentWithFb(contentTitle,contentUrl);
     * } else {
     * // Perform relevant action
     * }
     * break;
     ...
     * @param contentUrl   (String) : content url
     * @see <a href="https://developers.facebook.com/docs/sharing/android">Modeling Content</a>
     * //* @see #initFacebookShareDialog() share status
     */
    public void shareContentWithDialog(String contentTitle, String contentUrl) {

        if (!TextUtils.isEmpty(contentUrl) && URLUtil.isNetworkUrl(contentUrl)) {

            if (ShareDialog.canShow(ShareLinkContent.class)) {
                ShareLinkContent shareLinkContent = new ShareLinkContent.Builder()
                        .setQuote((!TextUtils.isEmpty(contentTitle)) ? contentTitle : "")
                        .setContentUrl(Uri.parse(contentUrl))
                        .build();
     ...
     * that user has installed app in their device or not. It will open web view automatically
     * and after logged in, user can share content with facebook.
     * <p>
     * <br>To post image (bitmap) with facebook you should convert image in bitmap.</br>
     * <pre
     * {@code
     * public void onClickEvent(View view) {
     * switch (view.getId()) {
     * case R.id.xxx:
     * if (Util.checkInternetConnection()) {
     * FacebookIntegration.getInstance(activityContext).shareLinkContentWithFb(bitmap);
     * } else {
     * // Perform relevant action
     * }
     * break;
     ...
     * }</pre>
     *
     * @param bitmap : bitmap that to be shared with facebook
     *               //* @see FacebookIntegration#initFacebookShareDialog() share status
     */
    public void shareDynamicBitmapWithDialog(Bitmap bitmap) {
        ...
        if (bitmap != null) {
            ...
            fbShareDialog.show(photoContent);
   ...
   /* public void shareDynamicBitmapWithFb(Bitmap bitmap, String caption) {
        ...
        if (bitmap != null && !TextUtils.isEmpty(caption)) {
            ...
            ShareApi.share(photoContent, new FacebookCallback<Sharer.Result>() {
                @Override
                public void onSuccess(Sharer.Result result) {
                    Debug.trace(BaseApplication.getInstance().getString(R.string.strShareDynamicBitmapAndCaptionWithFb));
                }
